...
Rather, it is the relying party's choice of trusted CAs that determines interceptibility. If the relying party trusts an attacker-controlled CA, the attacker can intercept the connection, whether the authenticating party uses that CA or another CA. In both cases, the attacker must present a different certificate, with a different public key. Conversely, if the relying party does not trust the attacker's CA, the attacker cannot use it to substitute in a different public key, independent of the authenticating party's choice of CA.
